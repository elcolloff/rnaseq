...
### rustar-aligner (experimental)

:::warning
rustar-aligner support is experimental. Validate results against the standard STAR path on a pilot subset before relying on it for production work.
:::

[rustar-aligner](https://github.com/scverse/rustar-aligner) is a Rust port of STAR that reuses STAR's command-line interface and on-disk genome index format. Enable it with `--use_rustar_star`:
...
When `--use_rustar_star` is set:

- Alignment is performed by rustar-aligner instead of STAR, using the same args. Outputs land in the standard `star_salmon/` / `star_rsem/` directories.
- A pre-built STAR index supplied via `--star_index` is reused as-is (rustar reads the STAR format). Indexes built from scratch are generated by rustar-aligner.
- Tie-breaking among equally-scoring multi-mapped reads differs from STAR's because rustar uses a ChaCha-based RNG (STAR uses Mersenne Twister), so a small fraction of multi-mapper primary/secondary assignments may differ. Pass `--runRNGseed` via `--extra_star_align_args` if reproducibility across reruns matters more than STAR-byte-equivalence.
